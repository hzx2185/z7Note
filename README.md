...
### 🤝 协作功能
  ...
  - **CardDAV 同步**:
    - **深度兼容性修复**：完美支持 macOS 和 iOS 原生通讯录应用
...
- **列表排序深度优化**: 
  - **活跃置顶**: 左侧笔记列表现在不仅按笔记时间排序，还会根据分类下最新笔记的活动情况自动将**活跃分类**置顶。
  - **自动展开**: 修改笔记时会自动展开所属分类，确保当前操作可见。
- **预览体验升级**:
  - **弹出预览窗**: 新增独立的大尺寸预览模态框（🖼️），支持沉浸式阅读。
  - **全屏模式改进**: 优化了分屏切换逻辑，支持一键开关。
- **界面细节精简**: 
  - 侧边栏操作按钮（新建、批量、去重等）进行了紧凑化处理，适配各种屏幕宽度。
  - 移除了冗余的打印按钮，推荐在全屏预览模式下直接使用浏览器打印（已做打印样式优化）。
