...
module.exports = {
    ...
    migrate: async (db) => {
        ...
        if (!hasTfaSecret) {
            ...
            db.log('迁移完成: users.tfa_secret 字段已添加');
        } else {
            db.log('迁移跳过: users.tfa_secret 字段已存在');
        }

        if (!hasTfaEnabled) {
            await db.exec(`ALTER TABLE users ADD COLUMN tfa_enabled INTEGER DEFAULT 0`);
            db.log('迁移完成: users.tfa_enabled 字段已添加');
        } else {
            db.log('迁移跳过: users.tfa_enabled 字段已存在');
        }
    }
};
