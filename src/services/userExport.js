...
// 正在执行备份的用户集合，防止并发重复执行
const performingBackups = new Set();
// 最近一次备份完成的时间记录 (username -> timestamp)
...
async function performUserBackup(username, backupConfig) {
  ...
  try {
    ...
    if (backupConfig.sendEmail && backupConfig.emailAddress) {
      try {
        const fileList = textFiles.map(f => `  - ${f.filename}`).join('\n') + (attachmentCount > 0 ? `\n  - ${attachmentCount} 个附件文件` : '');
        const emailBody = `您的数据已成功备份到 WebDAV
...
- ${username}-calendar.ics: 日历事件
- ${username}-todos.json: 待办事项
- ${username}-contacts.vcf: 联系人
...
如有问题，请联系管理员。`;
    ...
    performingBackups.delete(username);
  }
}
