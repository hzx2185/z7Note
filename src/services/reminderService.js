    ...
    if (!existing) {
      ...
      await db.run(
        ...
        [
          ...
          settings.todo_complete_to_event !== undefined ? settings.todo_complete_to_event : 0,
          settings.delete_todo_after_convert !== undefined ? settings.delete_todo_after_convert : 1
        ]
      );
    } else {
      // 更新现有设置
      await db.run(
        `UPDATE reminder_settings SET
         ...
         todo_complete_to_event = COALESCE(?, todo_complete_to_event),
         delete_todo_after_convert = COALESCE(?, delete_todo_after_convert),
         updatedAt = ?
         WHERE username = ?`,
        [
          ...
          settings.todo_complete_to_event,
          settings.delete_todo_after_convert,
